...
def get_building_blocks_new(atoms):
    ...
    s_count = np.sum(ps_dist<2.6, axis=0)
    s_count[s_count>=2]=0
    print("this is fraction of single S:", s_count.shape, len(satoms))
    print("this is the ratio", np.sum(s_count)/len(satoms))
...
if 1:
    #for i in [0,1,3,5]:
    for i in [2,3,4,5]:#:
        atoms = read(f"/home/yli/database/equ/na4p2s7/{i}_comp.xyz","300::10")
        #atoms = read(f"/home/yli/database/0324/na3ps4_2/{i}_full_traj.xyz",":551:10")
    ...
    #np.savetxt(f"/home/yli/database/glassy_na3ps4_rdf/na3ps4_0324_simple.txt", blocks_arr)
    np.savetxt(f"/home/yli/database/glassy_na3ps4_rdf/na4p2s7_test.txt", blocks_arr)
