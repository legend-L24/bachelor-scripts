...
class charge_eq():
    ...
    def rms(km, prd, natom, q2, g_ewald):
    ...
    # this function is written by lyt and ignore the part of forces.
    def calc_Eqeq_old(self):
        A = self.get_A()
        self.f = np.zeros((self.nAt,3))
        self.E = 0.5*self.q@A@self.q

    # this part is written by Martin
    def calc_Eqeq(self):
        self.E = 0
        self.f = np.zeros((self.nAt,3))
    ...
    def get_Abar(self):
        if self.pbc:
            ...
            A = np.column_stack((A, np.zeros(self.nAt+1)))
        else:
    ...
    def min_Charges(self):
        ...
        #print(res.x)
        self.q = res.x
    def fun_Etot_old(self, q):
        A = self.get_Abar()
        return 0.5*self.q@A@self.q
