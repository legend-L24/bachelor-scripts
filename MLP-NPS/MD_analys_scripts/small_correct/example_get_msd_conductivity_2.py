...
def read_dump(dumpfile, dtime = 200, end= -1, start = 600):
    ...
    print("this is atoms legenth:",len(atoms), len(lammps_dump_file_2_ase(dumpfile,d_species,l_remove_type)))
    print("Chemical composition: ", atoms[-1].get_chemical_formula())
    return atoms
...
path = "./database/triple_glass"
'''
for matter in ["na4p2s7","na7p3s11", "na3ps4", "naps3"]:
...
'''
dir_ls = ["na4p2s7_new", "na4p2s7_new_1" ,"na4p2s7_new_2"]
for dir_0 in dir_ls:
    dir_1 = f"database/{dir_0}"
    sig = []
    for i, T in zip(range(1,6),range(400,900,100)):
        dir_0 = join(dir_1, f"{i}/geom.dump")
        trj = read_dump(dir_0)
        sigma, sigma_av = get_msd_conductivity(trj, f"new_{dir_0}_{T}K", temp=T)
        sig.append([sigma, sigma_av])
    print(sig)
    plt.clf()
    sig_ls.append(sig)
    sig_arr = np.array(sig_ls);np.savetxt(f"{dir_0}.txt", sig_arr[:,:,1])
print("this is final result: ", sig_ls)
'''
from os.path import join
