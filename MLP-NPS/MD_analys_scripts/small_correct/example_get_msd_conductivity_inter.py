...
def read_dump(dumpfile, dtime = 200, end= -1, start = 600):#600
    step = int(1000/dtime)
    print("Analyse file: ",dumpfile)
    d_species = {2:15,1:3,3:16}; l_remove_type=[4]
...
path = "./datapoint"#"./bridson"
#path = "./database"
#for matter in ["naps3", "na4p2s7", "na7p3s11","na3ps4"]:
'''
for matter in ["na3ps4","na3ps4_1","na3ps4_2","na4p2s7","na4p2s7_1","na4p2s7_2"]:
...
#for matter in ["na3ps4","na3ps4_1","na3ps4_2","na4p2s7","na4p2s7_1","na4p2s7_2","na7p3s11", "na7p3s11_1", "na7p3s11_2"]:
for matter in ["na3ps4","na3ps4_1","na3ps4_2"]:#["na3ps4_bridson","na3ps4_bridson_1","na3ps4_bridson_2"]:
    #for matter in ["alpha_new", "beta_new", "gama_new"]:
    dir_ls = [join(path, matter)]
    sg_ls = []
    num = 0
    for dir_1 in dir_ls:
        sig = []
        for i, T in zip(range(1,7),range(450, 1050,100)):
            #dir_0 = join("database/"+dir, f"{i}/geom.dump")
            dir_0 = join(dir_1, f"{i}/geom.dump")
            trj = read_dump(dir_0)
            sigma, sigma_av = get_msd_conductivity(trj, f"{matter}_{num}_{T}K", temp=T)
            sig.append([sigma, sigma_av])
            print(sig)
