...
color_ls = ["m","b","g","r","c","k"]
for matter in ["beta_npt"]:#, "beta"]:
    dir_ls = [join(path, matter)]
    sg_ls = []
    num = 0
    for dir in dir_ls:
        sig = []
        for i, T in zip(range(1,7),range(400,1000,100)):
            dir_0 = join(dir, f"{i}/geom.dump")
            trj = read_dump(dir_0, dtime=1000, end=500, start=0)
            lines+=plt.plot(range(0,len(trj)*100,100), [atom.get_volume() for atom in trj],color = color_ls[i-1])

print("this is legend:", lines, labels)
'''
plt.legend(lines[::5],["400K"],loc="best")
plt.legend(lines[1::5],["500K"],loc="best")
plt.legend(lines[2::5],["600K"],loc="best")
plt.legend(lines[3::5],["700K"],loc="best")
plt.legend(lines[4::5],["800K"],loc="best")
'''
plt.tight_layout()
plt.savefig("crystal_volume_plot_"+matter+".png")
