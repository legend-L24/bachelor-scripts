...
class atomic_diffusion_tools(object):
    ...
    def _calc_current_einstein_interval(self,atoms,charge_dict,interval):
        ...
        ## add only LiCl ##
        inds.update({'licl':np.array(np.where(atoms[0].get_atomic_numbers() == 15)[0].tolist() +\
                                    np.where(atoms[0].get_atomic_numbers() == 16)[0].tolist())})
        ...
        for i in range(0,interval):
            ...
            qmsd_li_eff, qmsd_cl_eff = self._calc_eff_qmsd(step_save[15],step_save[16])
    ...
    def _calc_current_einstein_time_averaged(self,atoms,charge_dict,lag):
        ''' calculate the Einstein formulation of the current correlation averaged
            over a lag time of selected atom types from a list of atoms objects
            input:  atoms = list of atoms objects
                  charges = charges in a dictionary for use
    ...
    def _calc_msd_time_averaged(self,atoms,atypes,lag):
        ...
        from time import time
        length = int(len(atoms)*lag) #images using for averaging
        #length = int(len(atoms)*lag/10) #images using for averaging
        print(length, len(atoms))
        msd, inds = {}, {}
        for atype in atypes:
        ...
        for atype in atypes:
            ...
            #msd[atype] /= len(range(0,len(atoms)+1-length))*10
        return(msd)
    ...
    def _calc_disp_per_atom(self,atoms,atype):
    ...
    def _calc_msd(self,atoms,atype):
        ...
        print(len(ind))
        displ, dpos_max = self._get_max_displacement(atoms)
        #print('displ',len(displ),len(displ[0]))
        #print('dpos_max',dpos_max)
        msd = self._displ2msd(displ,ind)
        return(msd)

    def _calc_msd_per_atom(self,atoms,atype):
        ''' calculates the mean square displacement (msd) of a selected
            atom type from a list of atoms objects
            input:  atoms = list of atoms objects
                    atype = atom type for which to compute msd
