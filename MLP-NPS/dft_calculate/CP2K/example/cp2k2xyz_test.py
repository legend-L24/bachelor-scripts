...
def cp2kout_to_aseatoms(file_name):
    ...
    if ("PROGRAM STOPPED IN" in files[-2]) or ("PROGRAM STOPPED IN" in files[-1]):
        chemical_symbols=[]
        positions=[]
        forces=[]
        stress_tensor = []
...
for i in range(0, 5):
    #print(i)
    str_file = Path("./"+name+"__" + str(i))
    if str_file.exists():
        A = cp2kout_to_aseatoms(name+"__" +str(i))
        if A != 0:
            pos.append(A)
        else: print(i)
#pos = cp2kout_to_aseatoms("output_test_1")
print(len(pos))
write("110_cut_3"+".xyz", pos)
