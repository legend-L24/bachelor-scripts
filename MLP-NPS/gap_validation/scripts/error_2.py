...
print("this is legenth check: should be equal", len(atoms), len(atoms_))
frc_dft = np.array([])
frc_gap = np.array([])
ener_dft = []
ener_gap = []
for idx, atom in enumerate(atoms[:]):
    print(atoms_[idx].get_array('forces').shape,atom.get_array('force').shape)
...
print("this is force check:", frc_dft.shape, frc_gap.shape)
...
scale = (np.max(ener_dft)-np.min(ener_dft))/20
#scale = 0.4
#scale*=1.5
...
plt.savefig("energy_val_extra.png",bbox_inches = 'tight')
