...
int main (int argc, char const *const *argv) {
	...
	if (!(n[3] = cryst_dof (cr))) ERR_GOTO ("dof == 0", sched_err)
	cryst_ack (cr, 1);
	...
	//printf ("this is the size of crystal:%ld ", sizeof(crystal));

	printf ("12:00 I do it\n");
        printf (
	...
	for (unsigned i = 0, stable = 0;;) {
		init[7] = init[3];
		if (!sa_comp_get (comp, n, ctl, n + 4, init + 3)) ERR_GOTO
